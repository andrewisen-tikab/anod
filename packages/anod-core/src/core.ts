...
// ─── Prototype method installation ─────────────────────────────────
{
  ...
  SignalProto._assign = function (this: ISender, value: unknown, _time: number): void {
    this._value = value;
  };

  ComputeProto._assign = function (this: ICompute, value: unknown, time: number): void {
    this._value = value;
    this._ctime = time;
  };
  ...
  function suspend(this: ICompute | IEffect, promiseOrTask: unknown): unknown {
    ...
    if (typeof promiseOrTask === "function") {
      ...
      let node = this;
      let time = this._time;
      (promiseOrTask as (resolve: (v: unknown) => void, reject: (e: unknown) => void) => void)(
  ...
  ComputeProto.lock = EffectProto.lock = function (this: ICompute | IEffect): void {
    this._flag |= FLAG_LOCK;
  };

  ComputeProto.unlock = EffectProto.unlock = function (this: ICompute | IEffect): void {
    this._flag &= ~FLAG_LOCK;
  };
  ...
  function pending(this: ICompute | IEffect, tasks: ICompute | ICompute[]): boolean {
    let loading = false;
  ...
  SignalProto._changed = ComputeProto._changed = function (this: ISender, value: unknown): boolean {
  ...
  ComputeProto._settle = function (this: ICompute, value: unknown): void {
    ...
    if (value !== this._value || flag & (FLAG_INIT | FLAG_ERROR)) {
      ...
      if (this._flag & FLAG_ASYNC) {
        let hasDefers =
          this._flag & FLAG_CHANNEL &&
          ((this._args as IChannel)._defer1 !== null || (this._args as IChannel)._defers !== null);
        if (this._deps !== null || hasDefers) {
          stale = settleDeps(this);
        }
      }

      let waiters: (IReceiver | number | ((value: unknown) => void))[] | null = null;
      if (this._flag & FLAG_CHANNEL) {
      ...
      if (
        stale ||
        (flag & FLAG_LOCK &&
          (this._flag & FLAG_STALE || (this._flag & FLAG_PENDING && needsUpdate(this, TIME))))
      ) {
        this._flag |= FLAG_STALE;
        this._update(TIME);
      }
  ...
  ComputeProto._update = function (this: ICompute, time: number): void {
    ...
    let value: unknown;
    let args = flag & FLAG_CHANNEL ? (this._args as IChannel)._args : this._args;
    ...
    if ((flag & (FLAG_STABLE | FLAG_SETUP)) === FLAG_STABLE) {
      run: try {
        if (flag & FLAG_BOUND) {
          ...
          value = (this._fn as unknown as (c: ICompute, prev: unknown, args: unknown) => unknown)(
            this,
            this._value,
            args,
          );
    ...
    if (flag & FLAG_ASYNC) {
      ...
      if (kind !== ASYNC_SYNC) {
        ...
        if (kind === ASYNC_PROMISE) {
          ...
          resolveIterator(this, value as AsyncIterator<unknown> | AsyncIterable<unknown>, time);
        }
        return;
      }
    }
  ...
  EffectProto._update = function (this: IEffect, time: number): void {
    ...
    if ((flag & (FLAG_STABLE | FLAG_SETUP)) === FLAG_STABLE) {
      try {
        if (flag & FLAG_BOUND) {
          ...
          value = (this._fn as unknown as (val: unknown, c: IEffect, args: unknown) => unknown)(
            dep._value,
            this,
            args,
          );
        } else {
          value = (this._fn as unknown as (c: IEffect, args: unknown) => unknown)(this, args);
        }
      } finally {
    ...
    if (flag & FLAG_ASYNC) {
      ...
      if (kind !== ASYNC_SYNC) {
        this._flag |= FLAG_LOADING;
        if (kind === ASYNC_PROMISE) {
          resolvePromise(this as unknown as ICompute, value as PromiseLike<unknown>, time);
        } else {
          resolveIterator(
  ...
  function _compute(
    ...
    if (typeof depOrFn === "function") {
      ...
      flag = FLAG_STABLE | FLAG_BOUND | FLAG_SINGLE | ((0 | (argsOrOpts as number)) & OPTIONS);
      node = new _Compute(flag, fnOrSeed as ComputeFn, depOrFn as ISender, optsOrSeed, args);
  ...
  function _effect(
    ...
    if (typeof depOrFn === "function") {
      ...
      node = new _Effect(flag, depOrFn as EffectFn, null, this as Owner, optsOrArgs);
    } else {
      flag = FLAG_STABLE | FLAG_BOUND | FLAG_SINGLE | ((0 | (optsOrArgs as number)) & OPTIONS);
      node = new _Effect(flag, fnOrOpts as EffectFn, depOrFn as ISender, this as Owner, args);
      node._dep1slot = connect(depOrFn as ISender, node, -1);
    }
...
function patchDeps(node: IReceiver, version: number, depCount: number, newLen: number): void {
  let deps = node._deps;
...
function sweepDeps(stamp: number, dep1: ISender | null, deps: (ISender | number)[] | null): number {
  let depCount = 0;
  let vstack = VSTACK;
  let vcount = VCOUNT;
...
function resolvePromise(node: ICompute, promise: PromiseLike<unknown>, time: number): void {
  promise.then(
    (val) => {
      if (node._time !== time || (node._flag & FLAG_DISPOSED && !(node._flag & FLAG_LOCK))) {
        return;
...
function settleDeps(node: IReceiver): boolean {
  ...
  if (defer1 !== null) {
    if (defer1._version === stamp) {
      if ((defer1 as ISender & { _changed(v: unknown): boolean })._changed(defer1val)) {
        changed = true;
      }
    } else {
  ...
  for (let i = 0; i < deferLen; i += 2) {
    ...
    if (sender._version === stamp) {
      if ((sender as ISender & { _changed(v: unknown): boolean })._changed(snapshot)) {
        changed = true;
      }
      continue;
    }
...
function removeWaiter(responder: ICompute, responderCh: IChannel, slot: number): void {
  let waiters = responderCh._waiters!;
          ...
          for (let j = 0; j < count; j++) {
            ...
            if (node._flag & FLAG_STALE || (node._flag & FLAG_PENDING && needsUpdate(node, time))) {
              try {
                ...
                let e: unknown = node._flag & FLAG_PANIC ? err : { error: err, type: FATAL };
                node._flag &= ~FLAG_PANIC;
...
function effect(
  ...
  if (typeof depOrFn === "function") {
    ...
    flag = FLAG_STABLE | FLAG_BOUND | FLAG_SINGLE | ((0 | (optsOrArgs as number)) & OPTIONS);
    node = new _Effect(flag, fnOrOpts as EffectFn, depOrFn as ISender, null, args);
    node._dep1slot = connect(depOrFn as ISender, node, -1);
  }
  startEffect(node);
  return node;
