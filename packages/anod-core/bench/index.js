...
import {
  createState as zeixState,
  createMemo as zeixMemo,
  createEffect as zeixEffect,
...
function setupDiamond(fw) {
  return fw.withBuild(() => {
    ...
    const sum = fw.computed(() => branches.reduce((a, b) => a + b.val(), 0));
    fw.effect(() => {
      const v = sum.val();
      counter += v;
      sink += counter;
...
function setupTriangle(fw) {
  return fw.withBuild(() => {
    ...
    const sum = fw.computed(() => list.reduce((a, b) => a + b.val(), 0));
    fw.effect(() => {
      const v = sum.val();
      counter += v;
      sink += counter;
...
function setupMolWire(fw) {
  return fw.withBuild(() => {
    ...
    const D = fw.computed(() => numbers.map((i) => ({ x: i + (A.val() % 2) - (B.val() % 2) })));
    const E = fw.computed(() => hard(C.val() + A.val() + D.val()[0].x, "E"));
    const F = fw.computed(() => hard(D.val()[2].x || B.val(), "F"));
    const G = fw.computed(() => C.val() + (C.val() || E.val() % 2) + D.val()[4].x + F.val());
    fw.effect(() => {
...
const anodBench = {
  ...
  setupDiamond() {
    ...
    const sum = anodCompute((c) => branches.reduce((a, b) => a + c.read(b), 0));
    anodEffect((c) => {
      const v = c.read(sum);
      counter += v;
      sink += counter;
  ...
  setupTriangle() {
    ...
    const sum = anodCompute((c) => list.reduce((a, b) => a + c.read(b), 0));
    anodEffect((c) => {
      const v = c.read(sum);
      counter += v;
      sink += counter;
  ...
  setupCellx(layers) {
    ...
    for (let i = layers; i > 0; i--) {
      ...
      anodEffect((c) => {
        const v = c.read(s.prop1);
        counter += v;
        sink += counter;
      });
      anodEffect((c) => {
        const v = c.read(s.prop2);
        counter += v;
        sink += counter;
      });
      anodEffect((c) => {
        const v = c.read(s.prop3);
        counter += v;
        sink += counter;
      });
      anodEffect((c) => {
        const v = c.read(s.prop4);
        counter += v;
        sink += counter;
      });
      anodEffect((c) => {
...
const anodStableBench = {
  ...
  setupDiamond() {
    ...
    const sum = anodCompute((c) => branches.reduce((a, b) => a + c.read(b), 0), 0, OPT_STABLE);
    sum.watch((_, val) => {
      counter += val;
      sink += counter;
    });
  ...
  setupTriangle() {
    ...
    const sum = anodCompute((c) => list.reduce((a, b) => a + c.read(b), 0), 0, OPT_STABLE);
    sum.watch((_, val) => {
      counter += val;
      sink += counter;
    });
  ...
  setupMolWire() {
    ...
    const D = anodCompute(
      (c) => numbers.map((i) => ({ x: i + (c.read(A) % 2) - (c.read(B) % 2) })),
      null,
      OPT_STABLE,
    );
...
const kairoBenchmarks = [
  ...
  [
    ...
    setupAvoidable,
    anodBench.setupAvoidable,
    anodStableBench.setupAvoidable,
  ],
  [
    "repeated observers",
    setupRepeatedObservers,
    anodBench.setupRepeatedObservers,
    anodStableBench.setupRepeatedObservers,
  ],
];
