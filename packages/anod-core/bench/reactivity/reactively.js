...
/**
 * Reactively has lazy evaluation — effects only run on stabilize().
 * Wrap signal/compute/effect into shims that mirror the other frameworks.
 */
/** @param {any} v */
function signal(v) {
  return reactive(v);
...
/**
 * Reactively has no native batch. Run the block, then flush.
 * @param {() => void} fn
 */
function batch(fn) {
  fn();
...
/**
 * Build a rectangular reactive dependency graph using reactively API.
 * @param {number} width
...
/**
 * Build the graph once, force-read all leaves, then return a function that
 * writes one source and reads selected leaves per call.
 * @param {number} width
 * @param {number} totalLayers
 * @param {number} staticFraction
 * @param {number} nSources
