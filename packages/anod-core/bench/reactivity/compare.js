...
const allNames = new Set([...Object.keys(baseline.benchmarks), ...Object.keys(current.benchmarks)]);
...
for (const name of allNames) {
  ...
  let line =
    pad(name, nameW) +
    rpad(fmtTime(b.avg), colW) +
    rpad(fmtTime(c.avg), colW) +
    rpad(fmtDiff(b.avg, c.avg), diffW) +
    rpad(fmtBytes(b.heap), colW) +
    rpad(fmtBytes(c.heap), colW) +
    rpad(fmtDiff(b.heap, c.heap), diffW);
  if (hasCounters) {
    line +=
      rpad(fmtCount(b.instructions), colW) +
      rpad(fmtCount(c.instructions), colW) +
      rpad(fmtDiff(b.instructions, c.instructions), diffW);
    line +=
      rpad(fmtCount(b.cycles), colW) +
      rpad(fmtCount(c.cycles), colW) +
      rpad(fmtDiff(b.cycles, c.cycles), diffW);
  }
  console.log(line);
}
