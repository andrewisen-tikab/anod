...
declare module "anod-core/internal-types" {
  interface ISender<T> {
    ...
    includes(searchElement: unknown, fromIndex?: unknown, mutation?: boolean): ICompute;
    indexOf(searchElement: unknown, fromIndex?: unknown, mutation?: boolean): ICompute;
...
type ArrayCb<T = unknown> = (value: T, index: number, array: T[], node: ICompute) => unknown;
...
function sort(node: ISender, compareFn: unknown): void {
  (node._value as unknown[]).sort(compareFn as ((a: unknown, b: unknown) => number) | undefined);
  setMod(node, encode(MUT_SORT, 0, 0));
}
...
function isSignal(v: unknown): boolean {
  return v !== null && typeof v === "object" && (v as ISender)._flag !== undefined;
}

function getVal(arg: unknown): unknown {
  if (arg !== null && typeof arg === "object" && (arg as ISender)._flag !== undefined) {
...
function read(source: unknown): unknown {
  if (source !== null && typeof source === "object" && (source as ISender)._flag !== undefined) {
...
const _Compute = Compute as unknown as {
  new (opts: number, fn: ComputeFn, dep1: ISender | null, seed: unknown, args: unknown): ICompute;
};
const _Effect = Effect as unknown as {
  new (opts: number, fn: EffectFn, dep1: ISender | null, owner: unknown, args: unknown): IEffect;
};

function computeArray(source: ISender, fn: ComputeFn, args: unknown, opts?: number): ICompute {
...
function at(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown {
  return source.at(typeof args === "number" ? args : (read(args) as number));
}

SignalProto.at = ComputeProto.at = function (this: ISender | ICompute, index: unknown): ICompute {
  return computeArray(this as ISender, at as ComputeFn, index, isSignal(index) ? OPT_SETUP : 0);
};

function concat(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown[] {
  return source.concat(args as unknown[]);
}

function concatN(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown[] {
...
SignalProto.concat = ComputeProto.concat = function (
  ...
  if (len === 1) {
    let item = items[0];
    return computeArray(this as ISender, concat as ComputeFn, item, isSignal(item) ? OPT_SETUP : 0);
  }
  return computeArray(this as ISender, concatN as ComputeFn, items);
};

function entries(source: unknown[], _node: ICompute): IterableIterator<[number, unknown]> {
  return source.entries();
}

SignalProto.entries = ComputeProto.entries = function (this: ISender | ICompute): ICompute {
  return computeArray(this as ISender, entries as ComputeFn, undefined);
};

function every(source: unknown[], _node: ICompute, prev: boolean, cb: ArrayCb): boolean {
...
function filter(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): unknown[] {
  let result: unknown[] = [];
  for (let i = 0; i < source.length; i++) {
    if (cb(source[i], i, source, _node)) {
...
SignalProto.filter = ComputeProto.filter = function (
  ...
  return computeArray(this as ISender, filter as unknown as ComputeFn, cb, opts);
};

function find(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): unknown {
  for (let i = 0; i < source.length; i++) {
    if (cb(source[i], i, source, _node)) {
      return source[i];
    }
...
function find_mut(source: unknown[], _node: ICompute, prev: unknown, args: FindMutArgs): unknown {
  let cb = args._val;
  if (!(_node._flag & FLAG_INIT)) {
    let mod = _node._getMod!();
    if (mod > 0) {
...
function findIndex(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): number {
  for (let i = 0; i < source.length; i++) {
    if (cb(source[i], i, source, _node)) {
      return i;
    }
...
function findIndex_mut(source: unknown[], _node: ICompute, prev: number, cb: ArrayCb): number {
  if (!(_node._flag & FLAG_INIT)) {
    let mod = _node._getMod!();
    if (mod > 0) {
      let op = mod & MUT_OP_MASK;
...
function findLast(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): unknown {
...
SignalProto.findLast = ComputeProto.findLast = function (
  ...
  return computeArray(this as ISender, findLast as unknown as ComputeFn, cb, opts);
};

function findLastIndex(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): number {
  for (let i = source.length - 1; i >= 0; i--) {
    if (cb(source[i], i, source, _node)) {
      return i;
    }
...
function findLastIndex_mut(source: unknown[], _node: ICompute, prev: number, cb: ArrayCb): number {
  if (!(_node._flag & FLAG_INIT)) {
    let mod = _node._getMod!();
    if (mod > 0) {
      let op = mod & MUT_OP_MASK;
...
function flat(source: unknown[], _node: ICompute, seed: unknown, depth: unknown): unknown[] {
...
SignalProto.flat = ComputeProto.flat = function (
  ...
  return computeArray(this as ISender, flat as ComputeFn, depth, isSignal(depth) ? OPT_SETUP : 0);
};

function flatMap(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): unknown[] {
  let result: unknown[] = [];
  for (let i = 0; i < source.length; i++) {
...
SignalProto.flatMap = ComputeProto.flatMap = function (
  ...
  return computeArray(this as ISender, flatMap as unknown as ComputeFn, cb, opts);
};
...
SignalProto.forEach = ComputeProto.forEach = function (
  ...
  let node = new _Effect(flag, forEach as unknown as EffectFn, this as ISender, null, cb);
...
function includes1(source: unknown[], _node: ICompute, seed: unknown, arg: unknown): boolean {
...
function includes2(source: unknown[], _node: ICompute, seed: unknown, args: unknown): boolean {
...
function indexOf1(source: unknown[], _node: ICompute, seed: unknown, arg: unknown): number {
  return source.indexOf(getVal(arg));
}

function indexOf1_mut(source: unknown[], _node: ICompute, prev: number, arg: unknown): number {
  if (!(_node._flag & FLAG_INIT)) {
    let mod = _node._getMod!();
    if (mod > 0) {
      let op = mod & MUT_OP_MASK;
...
function indexOf2(source: unknown[], _node: ICompute, seed: unknown, args: unknown): number {
  let arr = args as unknown[];
...
function join(source: unknown[], _node: ICompute, seed: unknown, separator: unknown): string {
  return source.join(separator !== undefined ? (getVal(separator) as string) : undefined);
}
...
SignalProto.keys = ComputeProto.keys = function (this: ISender | ICompute): ICompute {
  return computeArray(this as ISender, keys as ComputeFn, undefined, 0);
};

// --- map ---

function map(source: unknown[], _node: ICompute, seed: unknown, cb: ArrayCb): unknown[] {
...
function reduce1(source: unknown[], _node: ICompute, seed: unknown, arg: unknown): unknown {
  let cb = arg as (
    acc: unknown,
    val: unknown,
    i: number,
...
function reduce2(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown {
  let arr = args as unknown[];
  let cb = arr[0] as (
    acc: unknown,
    val: unknown,
...
SignalProto.reduce = ComputeProto.reduce = function (
  ...
  return computeArray(this as ISender, reduce2 as unknown as ComputeFn, [cb, initialValue], opts);
};

// --- reduceRight ---

function reduceRight1(source: unknown[], _node: ICompute, seed: unknown, arg: unknown): unknown {
  let cb = arg as (
    acc: unknown,
    val: unknown,
    i: number,
...
function reduceRight2(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown {
  let arr = args as unknown[];
  let cb = arr[0] as (
    acc: unknown,
    val: unknown,
...
function slice1(source: unknown[], _node: ICompute, seed: unknown, arg: unknown): unknown[] {
  return source.slice(getVal(arg) as number);
}

function slice2(source: unknown[], _node: ICompute, seed: unknown, args: unknown): unknown[] {
  let arr = args as unknown[];
...
SignalProto.slice = ComputeProto.slice = function (
  ...
  return computeArray(this as ISender, slice2 as unknown as ComputeFn, [start, end], 0);
};

// --- some ---

function some(source: unknown[], _node: ICompute, prev: boolean, cb: ArrayCb): boolean {
  if (!(_node._flag & FLAG_INIT)) {
    let mod = _node._getMod!();
    if (mod > 0) {
      let op = mod & MUT_OP_MASK;
...
SignalProto.values = ComputeProto.values = function (this: ISender | ICompute): ICompute {
...
SignalProto.fill = function (this: ISender, value: unknown, start?: number, end?: number): void {
  if (IDLE) {
...
SignalProto.sort = function (this: ISender, compareFn?: (a: unknown, b: unknown) => number): void {
  if (IDLE) {
